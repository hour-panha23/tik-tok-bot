...
def main():
    ...
    if webhook_url:
        # Run with webhooks for production (e.g., Render)
        print(f"Bot is running with webhooks on port {port}. Webhook URL: {webhook_url}")
        application.run_webhook(
            listen='0.0.0.0',
            port=port,
            webhook_url=webhook_url,
            secret_token=os.getenv('SECRET_TOKEN')  # Optional for security
        )
    else:
